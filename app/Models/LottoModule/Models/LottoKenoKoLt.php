...
class LottoKenoKoLt extends BasicModel
{
    use Lotto28Trait;

    public $rememberCacheTag = 'lotto_keno_ko_lt';

    protected $lotto_name = 'kojc28';

    protected $table = 'lotto_keno_ko_lt';
    ...
    public function lottoOpenItem($item)
    {
        ...
        dump($control_val);
        if ($count <= 10) {
            if (($item->control === 'bet' && $control_val >= 10) || $control_val >= 200) {
                $count += 1;
                goto start;
            }
        }
