...
class LottoKenoLuLt extends BasicModel
{
    use Lotto28Trait;

    public $rememberCacheTag = 'lotto_keno_lucky_lt';

    protected $lotto_name = 'lult28';

    protected $table = 'lotto_keno_lucky_lt';
    ...
    public function lottoCreate()
    {
        ...
        if ($next_time < time()) {
            $time_a    = time();
            $time_b    = strtotime('2019-06-08') + 30;
            $diff      = intval(($time_a - $time_b) / 30) + 1;
            $new_id    = 100000 + $diff;
            $next_time = $diff * 30 + $time_b;
        }
