    ...
    <table class='table_list' cellspacing='1' cellpadding='0'>
      <tbody>
        <tr bgcolor='#fbfbfb'>
          <th colspan='41' style='height: 30px;line-height: 30px;'>
            <div style='position: relative;'>加拿大16走势图 <select id='sltNum'
        ...
        <tr class='timeh'>
        <th colspan='2'><b class='black777'>{{$limit}}期内标准次数</b></th>
          @foreach ($result['pro_stand'] as $val)
          <th>{{$val}}</th>   
         @endforeach
          <th colspan='5' style='background: white;'>* <span style='color:#5D5D5D'>注:<span
          style='color:red;font-weight: bold;'>{{$limit}}</span> 期开奖次数</span></th>
        </tr>
        <tr class='timeh'>
        <th colspan=2><b class='black777'>{{$limit}}期内实际次数</b></th>
         @foreach ($result['pro_real'] as $val)
          <th>{{$val}}</th>   
         @endforeach
        ...
        <tr class='font_color_2' bgcolor='#e3f0ff'>
          <th width='50'>期号</th>
          <th width='60'>时间</th>
          @foreach ($result['code_place'] as $code)
            <th width='22'>{{$code}}</th>
          @endforeach
      ...
      <tbody>
        ...
        <tr>
          ...
          @foreach ($result['code_place'] as $code)
          <td @if($code<=7 || $code>=14)class='bgnum' @endif>@if($item['chart']['win_he'] == $code)<em
          ...
          @if($item['chart']['middle']) <td class='bgkai03'>中</td> @else <td></td> @endif
          @if($item['chart']['side']) <td class='bgkai04'>边</td> @else <td></td> @endif
          @if($item['chart']['big']) <td class='bgkai05'>大</td> @else <td></td> @endif
          @if($item['chart']['small']) <td class='bgkai06'>小</td> @else <td></td> @endif

          @if($item['chart']['mta_big']) <td class='bgkai07'>大</td> @else <td></td> @endif
          @if($item['chart']['mta_small']) <td class='bgkai08'>小</td> @else <td></td> @endif
          <td class='black333'>{{$item['chart']['mod_3']}}</td>
          <td class='black333'>{{$item['chart']['mod_4']}}</td>
          <td class='black333'>{{$item['chart']['mod_5']}}</td>
        </tr>
        @endforeach
      </tbody>
    </table>
...
<script type="text/javascript">
  $(document).ready(function () {
    $('#sltNum').change(function () {
      ...
      if (v != '') {
        window.location.href = baseUrl+'/api/lotto/chart?name=ca28&chart='+chart+'&limit=' + v;
      }
    });
  });
</script>